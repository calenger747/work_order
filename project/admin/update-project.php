<?php 
  ...
  $nomor = "-".$tahun;
    ...
    $q1 = "SELECT max(tbl_schedule_wo.kode_jadwal) as maxKode FROM tbl_schedule_wo JOIN tbl_project_wo ON tbl_project_wo.kode_jadwal = tbl_schedule_wo.kode_jadwal WHERE WHERE YEAR(tgl_project) = '$tahun' ";
  ...
  if ($p_tombol == "Update") {
    ...
    $q_tmb = mysqli_query($con, "INSERT INTO tbl_project_wo VALUES ('', '$newpro', '$newsch', '$newtch', '$p_level', '$p_date', '$p_project', '$p_id_wo$newwo', '$p_id_so', '$p_customer', '$p_lokasi', '$p_pic', '')");
    $q_tmb1 = mysqli_query($con, "INSERT INTO tbl_schedule_wo VALUES ('$newsch', '$p_jadwal')");
    $q_tmb2 = mysqli_query($con, "INSERT INTO tbl_teknisi_wo VALUES ('$newtch', '$p_teknisi1', '$p_teknisi2', '$p_teknisi3', '$p_teknisi4', '$p_pkl1', '$p_pkl2', '$p_pkl3')");
    
    if ($q_tmb && $q_tmb1 && $q_tmb2) {
      echo "<div class='alert alert-success'><strong>SUCCESS</strong> Data Order Berhasil di Update <i class=' ace-icon fa fa-check'></i><button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button><a href='?id=project' class='btn btn-primary' style='float: right;'><i class='ace-icon fa fa-folder-open'></i> Show Data</a></div>";
    } else {
